CREATE OR REPLACE PACKAGE BODY TABLEROS_PKG AS
    ...
    --Validar palabra
    PROCEDURE VALIDAR_PALABRA_PR(P_ID_TABLERO NUMBER, P_ID_USUARIO VARCHAR2, P_LETRAS VARCHAR2, P_PUNTOS OUT NUMBER) IS
    ...
    BEGIN
        SELECT T.IND_PRIMER_PALABRA INTO IND_PRIMER_PALABRA FROM TABLERO T WHERE ID = P_ID_TABLERO;
    ...
    --Eliminar la ficha de un jugador
    PROCEDURE ELIMINAR_LETRAS_PR(P_ID_TABLERO NUMBER, P_ID_USUARIO VARCHAR2, P_PALABRA VARCHAR2) IS
    BEGIN
        FOR i IN 1..LENGTH(P_PALABRA)
            LOOP
                DELETE
                FROM LETRAS_USUARIO_TABLERO
                WHERE ID_TABLERO = P_ID_TABLERO
                  AND ID_USUARIO = P_ID_USUARIO
                  AND ID_LETRA = SUBSTR(P_PALABRA, i, 1)
                  AND ROWNUM = 1;
            end loop;
        OTORGAR_FICHAS_PR(P_ID_TABLERO, P_ID_USUARIO);
    END;

    --Incrementar las estadísticas de los jugadores
    PROCEDURE INCREMENTAR_ESTADISTICA_PR(P_ID_USUARIO VARCHAR2, P_PARTIDAS_JUGADAS NUMBER, P_PARTIDAS_GANADAS NUMBER,
                                         P_PARTIDAS_PERDIDAS NUMBER) IS
    BEGIN
        UPDATE USUARIO SET PARTIDAS_JUGADAS = PARTIDAS_JUGADAS + P_PARTIDAS_JUGADAS WHERE NOMBRE = P_ID_USUARIO;
        UPDATE USUARIO SET PARTIDAS_GANADAS = PARTIDAS_GANADAS + P_PARTIDAS_GANADAS WHERE NOMBRE = P_ID_USUARIO;
        UPDATE USUARIO SET PARTIDAS_PERDIDAS = PARTIDAS_PERDIDAS + P_PARTIDAS_PERDIDAS WHERE NOMBRE = P_ID_USUARIO;
    END;

    --Validar ganador del juego
    PROCEDURE VALIDAR_GANADOR_PR(P_ID_TABLERO NUMBER) IS
        P1        VARCHAR(100) := '';
        P2        VARCHAR(100) := '';
        PUNTOS_P1 NUMBER       := 0;
        PUNTOS_P2 NUMBER       := 0;
    BEGIN
        SELECT JUGADOR_1 INTO P1 FROM TABLERO WHERE ID = P_ID_TABLERO;
        SELECT JUGADOR_2 INTO P2 FROM TABLERO WHERE ID = P_ID_TABLERO;

        SELECT PUNTAJE_JG_1 INTO PUNTOS_P1 FROM TABLERO WHERE ID = P_ID_TABLERO;
        SELECT PUNTAJE_JG_2 INTO PUNTOS_P2 FROM TABLERO WHERE ID = P_ID_TABLERO;

        IF (PUNTOS_P1 > PUNTOS_P2) THEN
            INCREMENTAR_ESTADISTICA_PR(P1, 0, 1, 0);
            INCREMENTAR_ESTADISTICA_PR(P2, 0, 0, 1);
        ELSIF (PUNTOS_P2 > PUNTOS_P1) THEN
            INCREMENTAR_ESTADISTICA_PR(P1, 0, 0, 1);
            INCREMENTAR_ESTADISTICA_PR(P2, 0, 1, 0);
        end if;
    END;
